...
import { IAddress, ApiResponse, IPincode, IPostOffice } from "@/types/types";
import { Label } from "@/components/ui/label";
import { Input } from "@/components/ui/input";
import { useSpecificAddress } from "@/services/useSpecificAddress";
...
const EditAddressPage = ({ params: { id } }: EditAddressPageProps) => {
  const router = useRouter();
  const { token } = AuthProvider();
  const { data, isLoading, error } = useSpecificAddress(token, id);
  const shippingAddress: IAddress = data?.data;
  ...
  // Get all the address details initially
  useEffect(() => {
    if (shippingAddress) {
  ...
  // On updating the address details
  const onUpdateAddress = (data: addressSchemaType) => {
    // console.log("Form submitted:", data);
  };

  if (isLoading) return <div>Loading...</div>;
  if (error) return <div>Failed to fetch the address details</div>;
